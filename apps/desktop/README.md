...
| Window chrome | A native application menu occupies its own row below the window title bar, so Windows and Linux showed a two-row header, and a document title in the window bar duplicates the session header the client already draws. | Every window but macOS hides the native title bar, keeps the system-drawn window controls over the page, and renders one injected title bar row holding the DeepSeek mark, the application menu, and the window drag region. |
...
macOS keeps its native title bar and renders the application menu in the system menu bar. Every other window sets `titleBarStyle: 'hidden'` with a transparent `titleBarOverlay`, so the system still draws the minimize, maximize, and close controls over the page while the title text is gone, and it renders no native menu bar. The preload of each window injects the title bar into its document: the DeepSeek mark drawn in the document's own text color, the application menu and Help buttons, a drag region, a page offset that reserves the bar's row, and a [stylesheet adopted through the CSSOM](../../.agents/notes/implemented/architecture/2026-09-11-desktop-in-window-title-bar.md) because the shell documents' `style-src 'self'` policy rejects `<style>` elements and style attributes. The application menu carries Desktop Plugins and Quit; Help carries Check for Updates and About, and both open as native popups below their button. The renderer reports its own title bar color and whether the document paints a dark surface, which the shell applies to the window-control glyphs and to `nativeTheme`, so native menus and dialogs follow the theme the application itself renders. The window icon is the repository's own whale mark, rendered into `build/icon.ico` for the installer and executable and `renderer/app-icon.png` for running windows.
...
The loading page does not depend on the Host. Errors offer restart and reinstallation guidance. Disabling plugins and resetting Desktop are offered only when packaged application resources support profile recovery; development and early initialization failures expose restart alone. The plugin manager remains available through the application menu in the window's title bar. Runtime identity is checked before any backend starts; plugin changes have no automatic rollback.
...
A packaged application checks its target-specific release stream ten seconds after the main window opens, and every four hours afterwards while it runs. An available release raises the title bar's update badge in every window instead of opening a dialog; pressing that badge downloads and verifies the Desktop release, stops the dsh child, and hands installation plus restart to electron-updater, and the badge reports download progress in place while that runs. The next launch displays the local loading page while reconciling the version-bound runtime.

The localized **Check for Updates…** menu item checks on demand and reports its outcome in a native dialog: the check itself lights the badge as it runs, an available release offers Install and Restart, a current installation reports that it is up to date, and a failed check offers a retry from the badge. A failed automatic check reports nothing, so an offline application stays quiet.
