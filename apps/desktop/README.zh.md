...
打包应用会在主窗口打开十秒后检查目标专用的发布流，并在运行期间每四小时再检查一次。发现可用版本时，应用不会弹窗，而是在每个窗口的标题栏点亮更新提示；按下该提示会下载并验证 Desktop 发布、停止 dsh 子进程，并把安装与重启交给 electron-updater，下载过程中提示会就地显示进度。下次启动在显示本地加载页的同时校准版本绑定的运行时。

本地化的 **检查更新…** 菜单项按需检查并用原生弹窗报告结果：检查进行时同样点亮标题栏提示，发现可用版本时提供「安装并重启」，已是最新时给出提示，检查失败时可以从提示上点击重试。自动检查失败不会显示任何内容，因此离线运行时应用保持安静。
